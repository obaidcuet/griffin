...
object DataConnectorFactory extends Loggable {
  ...
  val HiveExtRegex: Regex = """^(?i)hive-ext$""".r
  val HiveProfRegex: Regex = """^(?i)hive-prof$""".r
  ...
  def getDataConnector(
    ...
    Try {
      conType match {
        ...
        case HiveExtRegex() => HiveBatchDataConnectorExtension(sparkSession, dcParam, tmstCache)
        case HiveProfRegex() => HiveBatchProfileDataConnector(sparkSession, dcParam, tmstCache)
